...
\subsubsection{Pr\'erequis}
\begin{itemize}[leftmargin=*]
	\item compilateur C/C++ compatible C11/C++11 (GCC ou Clang) ;
	\item CMake ;
	\item GGML (inclus au d\'ep\^ot) ;
	\item KaHIP optionnel, install\'e dans un pr\'efixe local (e.g., \texttt{third\_party/kahip-install}).
\end{itemize}

\subsubsection{Proc\'edure de compilation}
...
En l'absence de KaHIP, la compilation reste possible avec \texttt{-DGGML\_USE\_KAHIP=OFF}.

\subsubsection{Ex\'ecution minimale}

\begin{verbatim}
./build/parser model.gguf
\end{verbatim}

Cette commande produit : (i) une construction du DAG, (ii) des indicateurs de co\^ut de calcul/communication, et (iii) une proposition de partition (selon backend de partitionnement actif).
